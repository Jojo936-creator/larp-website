...
export default function AdminAnnouncements({ user }) {
  ...
  useEffect(() => {
    axios.get('/api/announcements?channel=admin').then(res => {
      setAnnouncements(res.data.announcements.filter(a => a.channel === 'admin'));
    });
  }, []);
  ...
  const handlePost = async () => {
    ...
    setError('');
    try {
      const res = await axios.post('/api/announcements', { title, description, channel: 'admin' });
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 900, margin: '48px auto 0 auto', background: '#fff', borderRadius: 16, boxShadow: '0 4px 16px rgba(0,0,0,0.10)', padding: '40px 32px 32px 32px', border: '1px solid #e0e0e0', position: 'relative' }}>
        ...
        <p>Announcements reserved for admin, owner, and superowner.</p>
