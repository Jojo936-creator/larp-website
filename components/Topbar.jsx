...
const staffPages = [
  ...
  { href: '/manage_user', label: 'Manage User' }, // solo superowner
  { href: '/appeal-review', label: 'Appeal Review' }, // nuova voce per admin+
];
...
export default function Topbar({ user }) {
  ...
  const visiblePages = user
    ? staffPages.filter((page) => {
        if (page.href.includes('admin') && user.level === 'staff') return false;
        if (
          page.href.includes('owner') &&
          user.level !== 'owner' &&
          user.level !== 'superowner'
        )
          return false;
        if (page.href === '/manage_user' && user.level !== 'superowner') return false;
        if (
          page.href === '/appeal-review' &&
          !['admin', 'owner', 'superowner'].includes(user.level)
        )
          return false;
        return true;
      })
                    ...
                    {visiblePages.map((page) => (
                      <Link
                        ...
                        style={{
                          ...
                          background:
                            router.pathname === page.href ? '#f2f7ff' : 'transparent',
