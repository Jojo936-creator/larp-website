...
export default function AnnouncementsPage({ user, channel }) {
  ...
  const handlePost = async () => {
    ...
    setError('');
    try {
      const res = await axios.post('/api/announcements', { title, description, channel });
  ...
  return (
    <div style={{ maxWidth: 900, margin: '48px auto', padding: 32, background: '#fff', borderRadius: 16, boxShadow: '0 4px 16px rgba(0,0,0,0.1)' }}>
